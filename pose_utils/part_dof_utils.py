...
from rotations import noisy_rot_matrix, compute_rotation_matrix_from_matrix, compute_rotation_matrix_from_3d, compute_rotation_matrix_from_matrix_svd, compute_rotation_matrix_from_ortho6d
...
# def eval_part_model(gt, pred, axis=False):
...
#     rdiff = rot_diff_degree(gt['rotation'], pred['rotation'], axis=axis)
...
# only evaluate translation and rotation
def eval_part_model(gt, pred, axis=None, up_and_down_sym=False):
    tdiff = trans_diff(gt['translation'].squeeze(-1), pred['translation'].squeeze(-1))
    rdiff = rot_diff_degree(gt['rotation'], pred['rotation'], axis=axis, up_and_down_sym=up_and_down_sym)
...
def eval_part_full(gt, pred, per_instance=False, axis=None, up_and_down_sym=False):
    pdiff = eval_part_model(gt, pred, axis=axis, up_and_down_sym=up_and_down_sym)
...
def convert_pred_rtvec_to_matrix(pred, sym, rotation_kind):  # pred: [B, D] or [B, P, D]
    if sym:
        return compute_rotation_matrix_from_3d(pred.reshape(-1, pred.shape[-1])).reshape(pred.shape[:-1] + (3, 3))
    else:
        if rotation_kind == '6d':
            return compute_rotation_matrix_from_ortho6d(pred.reshape(-1, 6)).reshape(pred.shape[:-1] + (3, 3))
        elif rotation_kind == '9d':
            return compute_rotation_matrix_from_matrix_svd(pred.reshape(-1, 3, 3)).reshape(pred.shape[:-1] + (3, 3))
        else:
