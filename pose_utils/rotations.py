...
def matrix_to_R3(matrix, epsilon=1e-8):
    R3 = torch.zeros((matrix.shape[:-2]) + (3,)).to(matrix.device)
    theta = torch.acos(torch.clamp((matrix[...,0,0]+matrix[...,1,1]+matrix[...,2,2]-1)/2, min=-1, max=1))
    R3[..., 0] = 0.5 * (matrix[...,2,1] - matrix[...,1,2])/(torch.sin(theta) + epsilon) * (theta + epsilon)
    R3[..., 1] = 0.5 * (matrix[...,0,2] - matrix[...,2,0])/(torch.sin(theta) + epsilon) * (theta + epsilon)
    R3[..., 2] = 0.5 * (matrix[...,1,0] - matrix[...,0,1])/(torch.sin(theta) + epsilon) * (theta + epsilon)
    return R3


def R3_to_matrix(R3, epsilon=1e-8):
    shape = R3.shape
    R3 = R3.reshape(-1, 3)
    theta = torch.sqrt(torch.sum(R3 ** 2, dim=-1)+ epsilon).unsqueeze(-1)
    w_x = torch.zeros((R3.shape[0], 9)).to(R3.device)
    w_x[:, 1] = - R3[:, 2]
    w_x[:, 2] = R3[:, 1]
    w_x[:, 3] = R3[:, 2]
    w_x[:, 5] = - R3[:, 0]
    w_x[:, 6] = - R3[:, 1]
    w_x[:, 7] = R3[:, 0]
    w_x = w_x.reshape(-1, 3, 3)
    first_order = (torch.sin(theta) / theta).unsqueeze(-1).repeat(1,3,3) * w_x
    seconde_order = ((1 - torch.cos(theta)) / (theta ** 2)).unsqueeze(-1).repeat(1,3,3) * torch.bmm(w_x, w_x)
    mat =  first_order + seconde_order + torch.eye(3).to(R3.device) # BP, 3, 3
    mat = mat.reshape(shape[:-1] + (3, 3))
    return mat
...
def compute_rotation_matrix_from_matrix_svd(m):
  """
  buggy! It is really strange. If loss = 1*svd(raw_output*0+constant) or loss = 0*svd(raw_output*0+constant),
   gradient will be nan during backward. However, loss = 1*svd(raw_output*1+constant) is ok. Current solution is
   avoiding svd in loss.
